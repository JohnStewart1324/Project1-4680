...
const SimpleAIChat: React.FC<SimpleAIChatProps> = ({ isOpen, onClose, stocks }) => {
  ...
  const handleApiKeySubmit = (e: React.FormEvent) => {
    ...
    if (apiKey.trim()) {
      ...
      const successMessage: ChatMessage = {
        ...
        type: 'system',
        content: "✅ API key saved successfully! I'm now ready to provide AI-powered stock analysis.",
        timestamp: new Date(),
        source: 'system'
      }
  ...
  const generateStockAnalysisPrompt = (userQuery: string, stockData: Stock[]) => {
    const stockSummary = stockData.slice(0, 100).map(stock => ({
      ticker: stock.ticker,
      name: stock.name,
      price: stock.price,
      change: stock.change,
    ...
    return `You are Stockie, an expert stock analyst and financial advisor powered by Google Gemini. You have access to real-time stock data and should provide insightful, actionable advice.
...
9. Use the actual stock data provided, not general knowledge
...
Remember: You're Stockie, the friendly but knowledgeable stock expert! Use the provided data to give accurate, data-driven analysis.`
  }
    ...
    try {
      if (!isApiKeySet) {
        const errorMessage: ChatMessage = {
          ...
          type: 'assistant',
          content: "⚠️ Please set your Google Gemini API key first to use the AI chat feature. Enter your API key in the setup section above.",
          timestamp: new Date()
        }
        setMessages(prev => [...prev, errorMessage])
        setIsLoading(false)
      ...
      for (const modelName of modelsToTry) {
        try {
          ...
          console.log('📝 Using simple test prompt:', simplePrompt)
          
          const result = await model.generateContent(simplePrompt)
          const response = await result.response
          text = response.text()

          console.log(`✅ AI response received with ${modelName}:`, text)
          
          // If simple test works, try with the actual query
          if (text) {
            const actualPrompt = `You are Stockie, an AI stock expert. Please respond to this user query: "${inputValue}". Keep your response concise and helpful.`
            const actualResult = await model.generateContent(actualPrompt)
            const actualResponse = await actualResult.response
            text = actualResponse.text()
            console.log(`✅ Full response received with ${modelName}, length:`, text.length)
          }
          
          break // Success, exit the loop
        } catch (modelError) {
          console.warn(`⚠️ Model ${modelName} failed:`, modelError.message)
          console.warn(`⚠️ Full error:`, modelError)
